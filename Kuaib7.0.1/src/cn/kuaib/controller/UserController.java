...
public class UserController extends BaseController{
	...
	* */
    ...
    public String loginOut(HttpSession session, HttpServletRequest request){
        ...
        session.setAttribute("savesucss",""); //清除保存成功提示信息
        session.setAttribute("savefail",""); //清除保存失败信息
...
*/
   ...
   * */
   ...
   @ResponseBody
    public ModelAndView saveText(HttpServletRequest request,HttpSession session, Integer uid){
       boolean flag = false;
       User user = new User();
       user.setUid(((User)session.getAttribute(Constants.USER_SESSION)).getUid());
       String wangtext = request.getParameter("wangtext");
       logger.info("uid是>>>>!!!!!!!!!!!!!!!!!!!!!!!!!!!" + user.getUid());
       logger.info("wangtext是>>>>!!!!!!!!!!!!!!!!!!!!!!!!!!!" + wangtext);
       session.setAttribute("wangtext",wangtext);
       ModelAndView modelAndView = new ModelAndView();
       modelAndView.addObject("wangtext",wangtext);
       try {
           flag = userService.upText(user.getUid(),wangtext);
       } catch (Exception e) {
           e.printStackTrace();
       }
       if (flag == true){
           session.setAttribute("savesucss","保存成功！");
           session.setAttribute("savefail","");
           logger.info("插入文本成功！！！>>>>>>>>>>>>>");
           return new ModelAndView("main");
       }else {
           session.setAttribute("savesucss","");
           session.setAttribute("savefail","保存失败！");
           logger.info("插入文本失败！！！>>>>>>>>>>>>>>");
       }
       /*return "redirect:/sys/returnmain";*/
       return new ModelAndView("main");
   }
